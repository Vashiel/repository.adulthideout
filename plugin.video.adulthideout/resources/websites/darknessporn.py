...
try:
    ...
    xbmc.log(f"[AdultHideout] Vendor path inject failed in darknessporn.py: {e}", xbmc.LOGERROR)

try:
    import cloudscraper
    _HAS_CF = True
except Exception as e:
    xbmc.log(f"[DarknessPorn] cloudscraper import failed: {e}", xbmc.LOGERROR)
    _HAS_CF = False
...
class DarknessPorn(BaseWebsite):
    ...
    def __init__(self, addon_handle):
        super().__init__(
            name="darknessporn",
            base_url="https://darknessporn.com",
            search_url="https://darknessporn.com/?s={}",
            addon_handle=addon_handle
        )
        self.label = 'DarknessPorn'
        self._scraper_ua = (
