...
def process_heavy-r_content(url, page=1):
    if page == 1:
        add_dir(f'Search Heavy-r', 'heavy-r', 5, logos + 'heavy-r.png', fanart)
    ...
    try:
        ...
        if match:
            add_dir('[COLOR blue]Next Page >>>>[/COLOR]', 'heavy-r' + match[0], 2, logos + 'heavy-r.png', fanart)
    except:
        pass

def play_heavy-r_video(url):
    content = make_request(url)
