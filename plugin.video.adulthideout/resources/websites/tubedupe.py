...
class TubedupeWebsite(BaseWebsite):
    config = {
        "name": "tubedupe",
        "base_url": "https://www.tubedupe.com",
        "search_url": "https://www.tubedupe.com/search/?q={}",
        "categories_url": "https://www.tubedupe.com/categories/"
    }
        ...
        for attempt in range(max_retries):
            try:
                ...
                with opener.open(request, timeout=60) as response:
                    ...
                    self._save_debug_file(f'tubedupe_response_{url.replace("/", "_").replace(":", "_")}.html', content)
                    return content
            except urllib.error.HTTPError as e:
                self.logger.error(f"HTTP error fetching {url} (attempt {attempt + 1}/{max_retries}): {e}")
                if hasattr(e, 'read'):
                    error_content = e.read().decode('utf-8', errors='ignore')
                    self._save_debug_file(f'tubedupe_error_{url.replace("/", "_").replace(":", "_")}.html', error_content)
    ...
    def process_content_matches(self, content, current_url):
        pattern = r'<a href="([^"]+)" class="kt_imgrc" title="([^"]+)">.+?<img src="([^"]+)".+?<var class="duree">([:\d]+)</var>'
        matches = re.findall(pattern, content, re.DOTALL)
        if not matches:
            pattern_alt = r'<a href="([^"]+)"[^>]*title="([^"]+)"[^>]*>.*?<img src="([^"]+)".+?<span[^>]*class="[^"]*duration[^"]*">([:\d]+)'
            matches = re.findall(pattern_alt, content, re.DOTALL)
        ...
        next_page_match = re.search(r'<link href="([^"]*)" rel="next"/>', content)
    ...
    def play_video(self, url):
        ...
        try:
            match = None
            patterns = [
                (r"video_alt_url:\s*'([^']+)',[^']*video_alt_url_text:\s*'720p'", "Original video_alt_url pattern"),
            ...
            if not match:
                ...
                self._save_debug_file(f'tubedupe_error_{url.replace("/", "_").replace(":", "_")}.html', content)
            ...
            self.logger.error(f"Error processing video for URL {url}: {e}")
            self._save_debug_file(f'tubedupe_error_{url.replace("/", "_").replace(":", "_")}.html', content)
            self.notify_error(f"Error processing video: {e}")
