...
def search():
	try:
		...
		if 'ashemaletube' in name:
			...
			url = xhamster + '/search.php?qcat=video&q=' + searchText
			start(url)
...
def start(url): 
	...
	if 'ashemaletube' in url:
		...
		match = re.compile("<a class='thumb no_ajax' href='(.+?)'.+?data-original='(.+?)' alt=\"(.+?)\"><div class='videoDuration'>(.+?)</div>", re.DOTALL).findall(content)
		...
		match = re.compile('url\(\'http://(.+?)/(.+?)\'\) no-repeat;"><span>([:\d]+)</span></a>.+?<div colspan=2 class="vb_title"><a href="http://dato.porn/(.+?)" class="link"><b>(.+?)</b></a></div>', re.DOTALL).findall(content)
		for dummy, thumb, duration, url, name in match:
			thumb = thumb.replace('(\'','').replace(')','')
			dummy = 'http://' + dummy
			url = 'http://dato.porn/embed-' + url + '.html'
			content2 = make_request(url)
			match = re.compile('\|mp4\|(.+?)\|file\|', re.DOTALL).findall(content2)
			for url in match:
				url = dummy + '/' + url + '/v.mp4'
				add_link(name + ' [COLOR lime]('+ duration + ')[/COLOR]', datoporn + url, 4, dummy + '/' + thumb, fanart)	
		try:
			match = re.compile("<a href='([^']*)'>Next &rarr;").findall(content)
		...
		match = re.compile('href="/video/(.+?)">.+?data-original="(.+?)" title="(.+?)".+?class=".+?" width=".+?" height=".+?" />(.+?)<div class="viewsthumb">', re.DOTALL).findall(content)
		for url, thumb, name, dummy in match:
			if 'HD' in dummy:
				add_link(name + '[COLOR yellow]' +' [HD]' +'[/COLOR]', javtasty + '/video/' + url + 'OEHDOE', 4, thumb, fanart)
			else:
				add_link(name, javtasty + '/video/' + url, 4, thumb, fanart)
		try:
			match = re.compile('<a href="([^"]*)" class="prevnext">Next page').findall(content)
			add_dir('[COLOR blue]Next  Page  >>>>[/COLOR]', match[0], 2, logos + 'javtasty.png', fanart)
		except:
			pass
		...
		add_dir('[COLOR lime]Categories[/COLOR]', luxuretv + '/channel.php', 68, logos + 'luxuretv.png', fanart)
		...
		match = re.compile('<a href="([^"]+)" title="([^"]+)" class="img" data-related-url=".+?" >.+?data-mediumthumb="([^"]+)".+?<var class="duration">([^<]+)</var>(.+?)</div>', re.DOTALL).findall(content)
		...
		match = re.compile('<div class="widget-video-holder">.+?<a href="([^"]+)" title="([^"]+)".+?data-src="([^"]+)".+?<span class="widget-video-duration">.+?([:\d]+).+?</span>(.+?)</a><!--widget-video-link-->', re.DOTALL).findall(content)
		for url, name, thumb, duration, dummy in match:
			name = name.replace('&#39;', ' ').replace('&amp;', '&').replace('&quot;', '"').replace('	', '')
			if 'hd' in dummy:
		...
		match = re.compile('><a href="https://xhamster.com/movies/(.+?)".+?<img src=\'(.+?)\'.+?alt="([^"]*)".+?<b>(.+?)</b>.+?<div class="(.+?)"', re.DOTALL).findall(content)
		for url, thumb, name, duration, dummy in match:
			name = name.replace('&amp;', '&').replace('&quot;', '"').replace('&#39;', '\'')
			if '?from=video_promo' in url:
				pass
			if 'hSpriteHD' in dummy:
				add_link(name + '[COLOR yellow]' +' [HD]' +'[/COLOR]' +' [COLOR lime]('+ duration + ')[/COLOR]', 'http://xhamster.com/movies/' + url, 4, thumb, fanart)
			else:
		...
		if 'profiles' in url:
			...
			for url, thumb, dummy, url, name, duration in match:
				...
				if '>HD+<' in dummy:
					...
					add_link(name + ' [COLOR lime]('+ duration + ')[/COLOR]', xvideos + url, 4, thumb, fanart)
		try:
			match = re.compile('href="([^"]+)" class="no-page">Next').findall(content)
			match = [item.replace('&amp;', '&') for item in match]
		...
		for thumb, dummy, url, name, duration in match:
			if 'hd' in dummy:
				add_link(name + '[COLOR yellow]' +' [HD]' +'[/COLOR]', 'https://www.youjizz.com' + url, 4, 'https:' + thumb, fanart)
			else:
		...
		match = re.compile('<a href="/([^"]*)" title="([^"]*)"><img src="([^"]*)"(.+?)> ([:\d]+)</div>', re.DOTALL).findall(content)
...
def redtube_categories(url):
	...
	for url, name, thumb in match:
		add_dir(name, 'https://www.redtube.com'+  url, 2, "http:" + thumb, fanart) 		
...
def redtube_channels_cat(url): 
	...
	for url, name in match:
		add_dir(name, 'https://www.redtube.com' + '/channel/' + url, 11, logos + 'redtube.png', fanart)  		
...
def xvideos_pornstars(url) :
	...
	for thumb, url, name in match:
		thumb = thumb.replace('\\', '')
		add_dir(name, xvideos + url + '#_tabVideos,videos-new', 2, thumb, fanart)	
	try:
...
def javtasty_categories(url):
	home()
	content = make_request(url)
	match = re.compile('<a href="(.+?)".+?data-original="(.+?)" title="(.+?)"', re.DOTALL).findall(content)
	for url, thumb, name in match:
		add_dir(name, javtasty + url,  2, javtasty + thumb, fanart)
...
def datoporn_categories(url):
	...
	for url, thumb, duration in match:
		name = url
		name = name.replace('+',' ')
		add_dir(name + ' [COLOR lime]('+ duration + ')[/COLOR]', 'http://dato.porn/category/' + url,  2, thumb, fanart)
...
def resolve_url(url):
	...
	if 'xvideos' in url:
		...
		media_url = re.compile('{ file: "(.+?)", type: "video/mp4" }').findall(content)[0]
	elif 'xhamster' in url:
		...
		try:
			media_url = 'https:' + re.compile('<source src="(.+?)" type="video/mp4" label="720p">').findall(content)[0]
			media_url = media_url.replace('&amp;','&')
		except:
			try:
				media_url = 'https:' + re.compile('<source src="(.+?)" type="video/mp4" label="480p">').findall(content)[0]
				media_url = media_url.replace('&amp;','&')
			except:
		...
		try:
			...
			try:
				...
				media_url = re.compile('240: "([^"]+)"').findall(content)[0]
	elif 'heavy-r' in url:	
		...
		if 'OEHDOE' in url:
			media_url = re.compile('var video_.+?=\'(.+?)\';').findall(content)[1]
			media_url = media_url.replace('OEHDOE','')
		else:
			media_url = re.compile('var video_.+?=\'(.+?)\';').findall(content)[0]
		...
		media_url = url
		media_url = media_url.replace('http://dato.porn','')
	elif 'plyplv' in url:
