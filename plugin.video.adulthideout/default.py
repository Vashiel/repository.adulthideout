...
def start(url): 
	...
	if 'ashemaletube' in url:
		...
		try:
			match = re.compile('<a href="([^"]+)"><i class="fa fa-chevron-right"></i></a>', re.DOTALL).findall(content)
			add_dir('[COLOR blue]Next  Page  >>>>[/COLOR]', tubegalore + match[0], 2, logos + 'tubegalore.png', fanart)
		except:
			pass
		...
		match = re.compile('<a href="([^"]+)" class="js-thumb-pagination".+?<img src="([^"]+)" alt="([^"]+)"(.+?)<div class="thumb-pagination">.+?<div class="thumb__duration">([^<]+)</div>', re.DOTALL).findall(content)
		for url, thumb, name, dummy, duration in match:
			if 'HD' in dummy:
				add_link(name + '[COLOR yellow]' +' [HD]' +'[/COLOR]' +' [COLOR lime]('+ duration + ')[/COLOR]',  url,  4, thumb, fanart)
			else:
		...
		match = re.compile('<a class="thumbnail thumbnail-pagination" href="([^"]*)".+?<img src="([^"]+)" alt="([^"]+)">.+?<span class="mdi mdi-timer">(.+?)<div class="thumbnail__info"><div class="thumbnail__info__right">([:\d]+)</div>', re.DOTALL).findall(content)
		for url, thumb, name, dummy, duration in match:
			if 'HD' in dummy:
				add_link(name + '[COLOR yellow]' +' [HD]' +'[/COLOR]' +' [COLOR lime]('+ duration + ')[/COLOR]',  url,  4, thumb, fanart)
			else:
		...
		if 'profiles' in url:
			match = re.compile('<img src="([^"]*)".+?<a href="([^"]*)">([^"]*)</a></p>.+?<span class="duration">\((.+?)\)</span>', re.DOTALL).findall(content)
			for  thumb, url, name, duration in match:
				name = name.replace('&amp;', '&').replace('&quot;', '"').replace('&#39;', '`')
				url = url.replace('THUMBNUM/','')
				add_dir(name + ' [COLOR lime]('+ duration + ')[/COLOR]', xvideos + url, 4, thumb, fanart)
		else:
			...
			for url, thumb, dummy, name, duration in match:
				...
				url = url.replace('THUMBNUM/','')
				if 'HD' in dummy:
					add_link(name + '[COLOR yellow]' +' [HD]' +'[/COLOR]' +' [COLOR lime]('+ duration + ')[/COLOR]', xvideos + url, 4, thumb, fanart)
				else:
...
def redtube_categories(url):
	...
	for url, name, thumb in match:
		add_dir(name, redtube + url, 2, "http:" + thumb, fanart) 		
...
def xhamster_categories(url):
	home()
	content = make_request(url)
	match = re.compile('<a href="https://xhamster.com/channels/(.+?)">([^"]+)</a>').findall(content)
	for url, name in match:
		name = name.replace('&amp;', '&').replace('&quot;', '"').replace('&#39;', '´')
...
def	txxx_categories(url):
	home()
	content = make_request(url)
	match = re.compile('<a href="([^"]*)" class="thumb thumb--video" title="([^"]*)".+?src="([^"]*)".+?<div class="thumb-info">\(<b>(.+?)</b> videos\)</div>', re.DOTALL).findall(content)
	for url, name, thumb, duration in match:
		add_dir(name + ' [COLOR lime]('+ duration + ')[/COLOR]', url ,  2, thumb, fanart)		
...
def resolve_url(url):
	...
	if 'xvideos' in url:
		...
		media_url = 'http:' + re.compile('<source src="(.+?)" type="video/mp4">').findall(content)[0]
	elif 'hentaigasm' in url:
		media_url = re.compile('file: "(.+?)",').findall(content)[0]
